...
- [ ] 2. `InferenceEngine.dispatch()` 增加 `camera_ids` 过滤（预计 1h）
...
- [ ] 3. 新建 `backend/app/detectors/street.py`，实现 `StreetDetector(Detector)`（预计 2h）
...
- [ ] 4. 实现 `StreetDetector.detect(frame)` 推理与聚合（预计 2h）
...
- [ ] 5. `backend/app/api/ws.py` 新增 `/ws/street` 通道（预计 1h）
...
- [ ] 6. `backend/run.py::start_services()` 注册检测器（预计 15min）
...
- [ ] 11. `frontend/src/router/index.js` 新增 `/street` 路由（预计 10min）
  - `{ path: '/street', component: () => import('../views/StreetMonitor.vue') }`
  - 依赖：任务 7

- [ ] 12. `frontend/src/App.vue` 左侧菜单新增「街道监控」入口（预计 15min）
